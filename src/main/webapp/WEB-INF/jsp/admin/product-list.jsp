                ...
                <div class="box box-primary">
                  <div class="box-body">
                    <table id="my_order" class="table table-bordered table-striped">
                      ...
                      <tbody>
	                      <c:forEach items="${sessionScope.showAllBooks}" var="showAllBook">
	                      <tr>
	                        <td>${showAllBook.isbn}</td>
	                        <td>${showAllBook.title}</td>
	                        <td>2014-11-7 11:31</td>
	                        <td><span class="label bg-yellow">￥${showAllBook.price}</span></td>
	                        <c:choose>
	                        	<c:when test="${showAllBook.status=='0'}">
	                        <td><span class="label bg-red">已下架</span></td>
	                        	</c:when>
	                        <c:otherwise>
	                        	<td><span class="label bg-green">已上架</span></td>
	                        </c:otherwise>
	                        </c:choose>
	                        <td><a href="updStatus.action?isbn=${showAllBook.isbn}&status=1">上架</a>丨<a href="updStatus.action?isbn=${showAllBook.isbn}&status=0">下架</a></td>
	                      </tr>
	                      </c:forEach>
                      </tbody>
                      <tfoot>
   ...
   $('#my_order').DataTable({
     ...
     'columns': [
    	    ...
    	    { 'data': null , 'render': function (data, type, row, meta) {
    	    		return '<a href="../servlet/product-detail?isbn='+row.isbn+'">查看</a>丨<a href="../servlet/product-edit?isbn='+row.isbn+'">编辑</a>';
	    		}
    	    }
    	  ]
	 */
