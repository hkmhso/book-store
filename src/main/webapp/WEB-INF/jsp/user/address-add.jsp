...
$(function () {
    ...
    $('#bt-add').click(function(){
        ...
        console.log("address-add.jsp->>每点击一次添加地址后的TT_TOKEN："+_token);
        //用户还没登录，不能添加地址
        if(!_token){
            console.log("address-add.jsp->>用户还没登录，不能添加地址");
            window.location.href="${ctx}/address/no_login_address.action";
            return;
        }
        console.log("address-add.jsp->>用户登录了，可以添加地址了");
        //读取用户的输入——表单序列化
        ...
        $.ajax({
            async: true,
            type: 'POST',
            url: '${ctx}/address/addAddress.action',
            data: inputData,
